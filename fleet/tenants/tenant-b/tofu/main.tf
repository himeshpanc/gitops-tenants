# FLEET tenant (tenant-b) — tofu provisions a versioned tenant PLATFORM CONFIG bundle in
# OpenBao (a realistic multi-field config, not just a greeting). The ExternalSecret
# is repointed to secret/tenant-b-<version> via yaml-update. tofu talks only to OpenBao.
terraform {
  required_providers {
    vault = {
